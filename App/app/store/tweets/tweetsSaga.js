...
export function* fetchRetweet(action) {
  try{
    const response = yield call(fetch, GET_RETWEET_URL(action.payload), {
      method: 'POST',
    });
    const data = yield call([response, "json"]);
    
    yield put(fetchRetweetSuccess(data))
  }catch(er) {
    yield put(fetchRetweetError(er))
  }
}
